...
\begin{figure*}[!t]
	...
	\caption{System Model of the Proposed Semantic Broadcast System with Knowledge Base Asymmetric.}
	\label{system}
\end{figure*}
...
In the AWGN channel, the transmitted signal is affected only by independent Gaussian noise without fading or multipath effects. The received symbol is
\begin{equation}
	y = x + n,
\end{equation}
...
\end{equation}
where $h_i$ and $n_i$ are the channel gain and noise for receiver Bob$i$. 
...
\begin{equation}
	x = f_{\mathrm{CE}}(r),
\end{equation}
where $f_{\mathrm{CE}}(\cdot)$ is the channel encoding module. The channel encoder adopts dense layers with different output units.The system uses an autoencoder structure to protect key semantic information.
...
This paper conducts simulation experiments based on a text transmission task, and Bob$1$ is set as the target receiver by default. The network architecture is based on \cite{ref8}. The simulation parameter settings and performance analysis results are introduced sequentially below.

\subsection{Performance Metrics}
Since the system targets semantic information rather than bit-level accuracy, the Bilingual Evaluation Understudy (BLEU) \cite{ref10} is used to evaluate semantic similarity, from 1-gram to 4-gram. BLEU ranges in $[0,1]$, the higher the value is, the closer the similarity between the restored text and the original text will be.
