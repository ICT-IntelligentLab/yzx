...
class Channels():
    ...
    def Rayleigh_MRC(self, Tx_sig, n_var, num_antennas=5):
        shape = Tx_sig.shape
        Tx_flat = Tx_sig.view(shape[0], -1, 2)   # [B, S, 2]
        Rx_signals = [] 

        for _ in range(num_antennas):
            H_real = torch.normal(0, math.sqrt(0.5), size=[1]).to(device)  
            H_imag = torch.normal(0, math.sqrt(0.5), size=[1]).to(device)  

            H = torch.tensor([[H_real, -H_imag],
                              [H_imag,  H_real]], device=device, dtype=Tx_flat.dtype).squeeze(-1)  # [2,2]

            Tx_i = torch.matmul(Tx_flat, H)           # [B, S, 2]

            Rx_i = self.AWGN(Tx_i, n_var)             # [B, S, 2]

            Rx_signals.append((Rx_i, H))             

        combined = self._maximum_ratio_combining(Rx_signals, n_var)  # [B, S, 2]

        Rx_sig = combined.view(shape)
        return Rx_sig

    def _maximum_ratio_combining(self, Rx_signals, n_var):
 
        batch_size = Rx_signals[0][0].shape[0]
        num_antennas = len(Rx_signals)
        weights = []
        for Rx_antenna, H in Rx_signals:
            # 信道共轭转置
            H_conj = H.transpose(-1, -2).conj()
            weights.append(H_conj)
        
        # 对每个天线的信号进行加权并累加
        combined_signal = torch.zeros_like(Rx_signals[0][0])
        for i, (Rx_antenna, H) in enumerate(Rx_signals):
            weighted = torch.matmul(Rx_antenna, weights[i])
            combined_signal += weighted
        
        # 归一化因子（可选，取决于具体实现）
        # 这里我们使用简单的平均合并
        combined_signal = combined_signal / num_antennas
...
def train_step(model, src, trg, n_var, pad, opt, criterion, channel, mi_net=None):
    ...
    if channel == 'None' or channel == 'NoChannel':
        Rx_sig = Tx_sig  # 直接传递，不经过信道
    elif channel == 'AWGN':
        Rx_sig = channels.AWGN(Tx_sig, n_var)
    elif channel == 'Rayleigh':
        Rx_sig = channels.Rayleigh(Tx_sig, n_var)
    elif channel == 'Rayleigh_MRC':
        Rx_sig = channels.Rayleigh_MRC(Tx_sig, n_var)
    elif channel == 'Rician':
        Rx_sig = channels.Rician(Tx_sig, n_var)
    elif channel == 'Rician_MRC':
        Rx_sig = channels.Rician_MRC(Tx_sig, n_var)
    else:
        raise ValueError("Please choose from AWGN, Rayleigh, Rayleigh_MRC, Rician, Rician_MRC, or None")
